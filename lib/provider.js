  ...
  return {
    async upload(file) {
      try {
        ...
        const [image, logo] = await Promise.all([
          ...
          jimp.read(config.watermark),
        ]);

        logo.resize(image.bitmap.width * 0.2, jimp.AUTO);

        const x = image.bitmap.width - logo.bitmap.width - logo.bitmap.width * 0.2;
        const y = image.bitmap.height - logo.bitmap.height - logo.bitmap.width * 0.2;

        let result = image.composite(logo, x, y, { opacitySource: 0.6 });

        const mbuffer = await result.getBufferAsync(file.mime);
        await bucketFile.save(mbuffer, fileAttributes);
    ...
    async uploadStream(file) {
      try {
        ...
        const [image, logo] = await Promise.all([jimp.read(buffer), jimp.read(config.watermark)]);
